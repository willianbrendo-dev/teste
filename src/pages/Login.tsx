...
const Login = () => {
  ...
  const getUserTargetRoute = async (session: Session | null): Promise<string> => {
    try {
      ...
      console.log(`[Login] Verificando role para usuário: ${userEmail} (${userId})`);
      ...
      if (error) {
        console.error("[Login] Erro ao verificar role:", error);
        toast.error("Erro ao verificar permissões. Contate o administrador.");
        return "/";
      }

      console.log(`[Login] isPrintBridge: ${isPrintBridge} para ${userEmail}`);
      ...
      if (isPrintBridge) {
        ...
        console.log(`[Login] ℹ️ USUÁRIO PADRÃO/ATENDENTE - ${userEmail} → /`);
