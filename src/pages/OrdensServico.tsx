...
import { Plus, ArrowLeft, Eye, Download, Printer, Search, Filter, Edit, Trash2, ChevronDown, ChevronUp, Upload as UploadIcon, FileImage, Camera, ClipboardCheck, UserPlus, Building, Smartphone, ShieldCheck } from "lucide-react";
...
import { PrintWithMethodSelection } from "@/components/PrintWithMethodSelection";
import { PrintMethod } from "@/components/PrintMethodDialog";
import { useRemotePrint } from "@/hooks/use-remote-print";
import { generateOrdemServicoESCPOS } from "@/lib/printer/escpos-os-generator";
...
const OrdensServico = () => {
  ...
  const { print: remotePrint, status: printStatus, loading: printLoading } = useRemotePrint();
  ...
  const calculateValorRestante = () => {
    ...
    const entrada = parseFloat(formData.valor_entrada || "0");
    return (estimado - entrada).toFixed(2);
  };
  ...
  const handleRemotePrint = async (method: PrintMethod) => {
    if (!selectedOrdem || printLoading) return;

    // Buscar checklist relacionado à ordem
    ...
    try {
      ...
      if (checklist) {
        // Buscar nome do atendente que criou o checklist
        let atendenteNome = null;
        if (checklist.created_by) {
          const { data: profile } = await supabase
            .from("profiles")
            .select("nome")
            .eq("id", checklist.created_by)
            .single();
          
          atendenteNome = profile?.nome || null;
        }

        const components: Record<string, string> = {};
        const componentKeys = [
          'situacao_touch', 'alto_falante', 'auricular', 'carregador', 'conector_carga',
          'microfone', 'camera_frontal', 'camera_traseira', 'flash', 'fone_ouvido',
          'wifi', 'bluetooth', 'biometria', 'face_id', 'botao_home', 'botao_power',
          'botao_volume', 'sensor_proximidade', 'vibra_call', 'situacao_carcaca',
          'sim_chip', 'slot_sim', 'parafuso'
        ];

        componentKeys.forEach(key => {
          if (checklist[key]) {
            components[key] = checklist[key];
          }
        });
    ...
    if (method === "wifi") {
      // Impressão via navegador (window.print)
      ...
      if (printWindow) {
        printWindow.document.write(`
          ...
          <html>
            <head>
              <title>Ordem de Serviço #${selectedOrdem.numero}</title>
              <style>
                @media print {
      ...
      // Impressão via USB/Print Bridge
      const escposData = generateOrdemServicoESCPOS(ordemData, checklistData || undefined);
      await remotePrint(selectedOrdem.id, escposData);
    }
  };
              ...
              <Card key={ordem.id} className="hover:shadow-md transition-smooth">
                ...
                <CardContent>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <PrintWithMethodSelection
                      onPrint={(method) => {
                        setSelectedOrdem(ordem);
                        setTimeout(() => handleRemotePrint(method), 100);
                      }}
                      status={printStatus}
          ...
          <div className="space-y-4 pt-4">
            ...
            <Button
              onClick={() => {
                ...
                navigate(`/ordens-servico?tab=checklists&ordem=${selectedOrdem?.id}`);
              }}
