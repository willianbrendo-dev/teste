...
import { generateOrdemServicoESCPOS } from "./escpos-os-generator";
...
export class PrintBridgeTestSuite {
  ...
  private async testESCPOSGeneration(): Promise<void> {
    ...
    try {
      ...
      const osData = generateOrdemServicoESCPOS(mockOrdem);
  ...
  private async testMultipleOrdersLoad(): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < numOrders; i++) {
        ...
        const escposData = generateOrdemServicoESCPOS(mockOrdem);
        orders.push({
          index: i,
