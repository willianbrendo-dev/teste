...
import { Plus, Search, Trash2, Eye, Filter, ChevronDown, Edit, FileDown, RefreshCw } from "lucide-react";
...
type ComponentStatus = "funcionando" | "com_defeito" | "nao_testado" | "nao_possui";
type ChecklistType = "android" | "ios";
type ChecklistStatus = "pendente" | "concluido" | "cancelado" | "em_atraso";
...
export default function ChecklistsContent({ highlightOrdemId }: ChecklistsContentProps) {
  ...
  const { print: remotePrint, status: printStatus, loading: isPrinting } = useRemotePrint();
  ...
  const handleSubmit = async () => {
    ...
    const componentesList = selectedType === "ios" ? componentesIOS : componentesAndroid;
  ...
  const handleUpdateChecklist = async () => {
    ...
    const componentesList = editingChecklist.tipo === "ios" ? componentesIOS : componentesAndroid;
    const componentesPreenchidos = componentesList.filter(
      (comp) => formData[comp.key as keyof Checklist] != null
    );
  ...
  const handlePrint = async (checklist: Checklist) => {
    ...
    const escposData = generateChecklistESCPOS(checklistData);
    await remotePrint(checklist.ordem_servico_id, escposData);
  };

  const handleGeneratePDF = (checklist: Checklist) => {
    const doc = new jsPDF();
    const componentesList = checklist.tipo === "ios" ? componentesIOS : componentesAndroid;
    const statusMap = {
    ...
    <div className="space-y-6">
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <Button
                      variant="outline"
                      size="icon"
                      onClick={() => handleChangeStatus(checklist)}
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Novo Checklist {selectedType === "android" ? "Android" : "iOS"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(selectedType === "ios" ? componentesIOS : componentesAndroid).map((componente) => (
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedChecklist && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(selectedChecklist.tipo === "ios" ? componentesIOS : componentesAndroid).map((componente) => (
